...
@CheckInfo(name = "Motion",type =  "H", description = "Checks for invalid horizontal velocity.")
public class MotionH extends Check {
    public MotionH(PlayerData data) {
        super(data);
    }

    private int hitTicks, buffer;
    ...
    public void handle(Packet packet) {
        ...
        if (packet.isPosition() && vData.getTicksSinceVelocity() == 1) {
            ...
            if (Math.abs(data.getVelocityProcessor().getVelocityX()) < 0.005 ||
                    ...
                    Math.abs(data.getVelocityProcessor().getVelocityZ()) < 0.005 || data.getPositionProcessor().isTeleported()) return;
            ...
            if (takenVelocity < givenVelocity && !isExempt(ExemptType.WEB, ExemptType.LIQUID, ExemptType.NEAR_WALL)) {
                if (hitTicks >= 2) {
                    buffer += 20;
                    if (buffer > 30) {
                        fail("Horizontal=" + Math.max(0, MathUtil.preciseRound(takenVelocity / givenVelocity, 3) * 100));
                    }
                }
            } else {
