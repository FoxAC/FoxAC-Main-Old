...
@CheckInfo(name = "Speed", type = "D", description = "Checks if player is going faster than possible")
public final class SpeedD extends Check {
    ...
    public void handle(final Packet packet) {
        if (packet.isFlying()) {
            ...
            if (isExempt(ExemptType.ICE, ExemptType.SLIME) || data.getPositionProcessor().getSinceIceTicks() < 8) {
                airLimit += 0.34F;
                groundLimit += 0.34F;
            }
